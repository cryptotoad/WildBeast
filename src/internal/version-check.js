...
exec('git remote update').then(gitVerify).catch(async () => {
  ...
  const stable = await SA.get('https://raw.githubusercontent.com/cryptotoad/WildBeast/master/package.json')
  const exp = await SA.get('https://raw.githubusercontent.com/cryptotoad/WildBeast/experimental/package.json')
